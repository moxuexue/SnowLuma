...
export function NotificationOptIn({ selectedIds, onChange }: NotificationOptInProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error('load notification opt-ins failed', error);
      setLoadError(error instanceof Error ? error.message : '加载通知渠道失败');
      setChannels([]);
    }
  }, [api]);
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border bg-card/40 p-4">
      ...
      <SkeletonSwap
        ...
        reserve={40}
        label="通知渠道"
        className={channels !== null ? 'skeleton-swap-fluid min-h-[40px]' : ''}
      >
        {channels !== null ? (
          loadError ? (
